...
fun EditScreen(component: EditCardComponent) {
    ...
    Scaffold(
        topBar = { Topbar { component.onBackClick() } }) { paddingValues ->
        ...
        Box(
            ...
            WordScreen(word = word, editClick = { text, translate ->
...
private fun WordScreen(
    word: Word, editClick: (String, String) -> Unit, removeClick: (Long) -> Unit
) {
...
@Composable
fun WordFiled(text: String, label: String, onValueChange: (String) -> Unit){
    OutlinedTextField(
        modifier = Modifier
            .fillMaxWidth()
            .padding(8.dp),
...
private fun Topbar(onBackClick: () -> Unit) {
    TopAppBar(
        titleText = stringResource(R.string.edit_card), onBackClick = onBackClick
    )
}